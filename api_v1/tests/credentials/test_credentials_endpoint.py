...
class UserEndpointTestCase(TestCase):
    ...
    def test_user_data_in_payload(self):
        """POST /sign-in: Should return user data in payload when it's a valid user"""
        test_user_password = '1234'
        test_user = User.objects.create_user(username='test', email='test@gmail.com', 
                                             password=test_user_password)
        response = self.client.post(f'{URL_PREFFIX}/sign-in', 
        ...
        self.assertEquals(response.status_code, 200)
        self.assertJSONContains(response, {'access', 'refresh'})
        decodedAccessToken = jwt.decode(response.json()['access'], verify=False)
        self.assertIn('user', decodedAccessToken)
        expected_keys = {'username', 'name', 'email', 'level', 'registred_date'}
        self.assertJSONContains(decodedAccessToken['user'], expected_keys)
